const forwardReadMoreBTN = document.getElementsByClassName("read_more")[1];
const backwardReadMoreBTN = document.getElementsByClassName("read_more")[0];
const firstSpan = document.getElementsByTagName("span")[1];
const aboutUsText = [
...
forwardReadMoreBTN.addEventListener("click", () => {
  let index = aboutUsText.indexOf(firstSpan.textContent) + 1;
  if (index === aboutUsText.length) {
    index = 0;
  }
  firstSpan.textContent = aboutUsText[index];
});
